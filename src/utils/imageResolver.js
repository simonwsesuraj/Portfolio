// Utility to dynamically resolve asset URLs without requiring manual import statements
const assetModules = import.meta.glob('../assets/**/*', { eager: true, import: 'default' });

/**
 * Resolves an image/asset filename or path to its bundled URL.
 * Supports:
 * - Direct filenames (e.g. "WeatherApp.png", "resume.pdf")
 * - Subpaths (e.g. "projects/MealDB.png", "Certifications/cert.jpg")
 * - External URLs (e.g. "https://example.com/image.png")
 * - Public paths (e.g. "/hero.jpg")
 *
 * @param {string} assetPath - Filename, path, or URL of the asset
 * @param {string} fallback - Optional fallback URL or SVG
 * @returns {string} Bundled asset URL or fallback
 */
export function resolveAsset(assetPath, fallback = '') {
  ...
  // If it's already an external URL or root-relative public asset
  if (
  ...
  // 1. Direct path check in assetModules
  ...
  // 2. Basename search (e.g. "MealDB.png" matching "../assets/projects/MealDB.png")
  for (const [path, url] of Object.entries(assetModules)) {
...
/**
 * Returns a sleek SVG gradient placeholder when an image is not provided.
 */
